<template>
  <div v-if="el.show" class="row pt-2 pb-2" :class="{ 'bg-light': el.start }">

    <div class="col-2">
      <div class="nomer_schet" :class="{ 'start' : !el.start }">
        <template v-if="el.start">
          {{ el["nomer-sceta"] ?? "" }}
        </template>
        <template v-else>
          &nbsp;
      ...
      <template v-if="el.start">
        ...
        <span @click="show1 = !show1">{{ el.naimenovanie ?? "" }}</span>
      ...
      <template v-else> &nbsp; {{ el.firma ?? "" }} </template>
    ...
    <div class="col">{{ el["debet-na-naclo-perioda"] ?? "-" }}</div>
    <div class="col">{{ el["kredit-na-naclo-perioda"] ?? "-" }}</div>
    <div class="col">{{ el["oborot-debet"] ?? "-" }}</div>
    <div class="col">{{ el["oborot-kredit"] ?? "-" }}</div>
    <div class="col">{{ el["debet-na-konec-perioda"] ?? "-" }}</div>
    <div class="col">{{ el["kredit-na-konec-perioda"] ?? "-" }}</div>
  </div>

  <div v-if="1 == 2" class="row item pt-2 pb-2">
    <div class="col-10 col-sm-9 col-md-4 col-lg-5 name" @click="show1 = !show1">
      {{ el.naimenovanie ?? "" }}
      {{ el.dobavka ?? "" }}
    ...
    <div class="d-none d-md-block col-md-3 col-lg-3 xname">
      <!-- {{ el.mol }}<br /> -->
      <a :href="el.sait">{{ el.sait }}</a
      ><br />
      <a :href="'mailto:' + el.yashhik">{{ el.yashhik }}</a
      ><br />
      <!-- {{ el.yashhik }}<br /> -->
    </div>
