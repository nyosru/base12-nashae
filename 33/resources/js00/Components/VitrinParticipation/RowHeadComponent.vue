<template>
  <tr :style="el.start ? '' : 'background-color: rgba(0,0,0,0.05)'" 
    v-if="el.start || listScheta[el['nomer-sceta']] "
    @click=" el.start ? listScheta[el['nomer-sceta']] = !listScheta[el['nomer-sceta']] : ''"
  >
    <td>
      <span v-if="el.start">
        {{ el["nomer-sceta"] ?? "" }}
...
<svg
    ...
    <td class="tr" >{{ el["debet-na-naclo-perioda"] ?? "-" }}</td>
    <td class="tr" >{{ el["kredit-na-naclo-perioda"] ?? "-" }}</td>
    <td class="tr" >{{ el["oborot-debet"] ?? "-" }}</td>
    <td class="tr" >{{ el["oborot-kredit"] ?? "-" }}</td>
    <td class="tr" >{{ el["debet-na-konec-perioda"] ?? "-" }}</td>
    <td class="tr" >{{ el["kredit-na-konec-perioda"] ?? "-" }}</td>
  </tr>
</template>
